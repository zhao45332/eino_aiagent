// Package rag 封装 Milvus + Embedding + eino Retriever，对外提供 search_knowledge 所需的向量检索。
package rag
...
import (
	...
	"aiagent/internal/retriever"
	"aiagent/internal/vectorstore"
)

// Service 复用 Milvus 与向量化检索；最终回复由主 Agent 与 search_knowledge 工具链生成。
type Service struct {
	kb  *vectorstore.KB
	rag *retriever.MilvusRetriever
}
...
func New(ctx context.Context, cfg *config.Config) (*Service, error) {
	...
	r := retriever.NewMilvusRetriever(kb, emb, 8)
	return &Service{kb: kb, rag: r}, nil
}
...
func (s *Service) RetrieveContext(ctx context.Context, userQuery string) (string, error) {
